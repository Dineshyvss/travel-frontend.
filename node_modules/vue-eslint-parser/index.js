...
function analyzeVariablesAndExternalReferences(parserResult, parserOptions) {
    const scope = analyze(parserResult, parserOptions);
    return {
...
function parseVForExpression(code, locationCalculator, parserOptions) {
    ...
    try {
        ...
        const tokens = ast.tokens || [];
        const comments = ast.comments || [];
        const scope = analyzeVariablesAndExternalReferences(result, parserOptions);
...
function analyzeScriptSetupScope(scopeManager, templateBody, df, _parserOptions) {
...
function analyzeCompilerMacrosVariables(scopeManager) {
    const globalScope = scopeManager.globalScope;
    if (!globalScope) {
        return;
    }
    const compilerMacroVariables = new Map();
...
function parseAsSFC(code, options) {
    ...
    if (options.eslintScopeManager) {
        if (scripts.some(isScriptSetupElement)) {
            ...
            analyzeScriptSetupScope(result.scopeManager, templateBody, rootAST);
        }
    }
    return {
        result,
